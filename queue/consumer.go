...
func (w *DispatchWorker) handle(workerID int, d amqplib.Delivery) {
	...
	if env.Cost > 0 {
		...
		if !result.Success {
			...
			log.Warnf("Insufficient funds (balance=%.4f, cost=%.4f) — dead-lettering",
		...
		log.Debugf("Deducted %.4f — balance_after=%.4f", env.Cost, result.BalanceAfter)
	}
...
func (w *DispatchWorker) markSent(outboxID uint64, providerMsgID string) error {
	...
	return w.db.Model(&models.Outbox{}).Updates(map[string]interface{}{
		"status":     "SENT",
		"message_id": providerMsgID,
	}).Where("id = ?", outboxID).Error
}
...
func (w *DispatchWorker) markFailed(outboxID uint64, reason string) {
	...
	if err := w.db.Model(&models.Outbox{}).Update("status", "FAILED").
		Where("id = ?", outboxID).Error; err != nil {
		logrus.Errorf("[DispatchWorker] markFailed outbox_id=%d reason=%s: %v", outboxID, reason, err)
	}
...
func (w *DispatchWorker) refund(clientID string, amount float64, reason string) {
	if amount <= 0 || clientID == "" {
		return
	}
	if err := w.hotWallet.Refund(context.Background(), clientID, amount, reason); err != nil {
		logrus.Errorf("[DispatchWorker] Refund failed client=%s amount=%.4f reason=%s: %v",
